...
DOCKER_IMAGE_NAME := $(DOCKER_REGISTRY)$(DOCKER_ORG)docker-tools
...
generate-sbom:
	syft $(DOCKER_IMAGE_NAME):$(IMMUTABLE_DOCKER_TAG) \
		-o spdx-json \
		--file ./artifacts/docker-tools-$(VERSION)-SBOM.json
...
publish-sbom: generate-sbom
	ghr \
		...
		-t $${GITHUB_TOKEN} \
		-n ${VERSION} \
		${VERSION} ./artifacts/docker-tools-$(VERSION)-SBOM.json
